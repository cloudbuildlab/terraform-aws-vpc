...
resource "aws_vpc_endpoint" "logs" {
  ...
  security_group_ids = [
    aws_security_group.logs_endpoint[0].id
  ]
...
resource "aws_vpc_endpoint" "monitoring" {
  ...
  security_group_ids = [
    aws_security_group.monitoring_endpoint[0].id
  ]

  tags = merge(
    {
      Name = "${var.vpc_name}-monitoring-endpoint"
      Type = "interface"
    },
    var.tags
  )
...
resource "aws_vpc_endpoint" "ssm" {
  ...
  security_group_ids = [
    aws_security_group.ssm_endpoint[0].id
  ]
...
resource "aws_security_group" "logs_endpoint" {
  ...
  name        = "${var.vpc_name}-logs-endpoint"
  description = "Security group for CloudWatch Logs endpoint"
  vpc_id      = aws_vpc.this.id
